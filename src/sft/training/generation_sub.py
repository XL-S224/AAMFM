...
def iterative_sampling_raw(
    ...
    output_tokens_list = client.batch_generate(input_tokens, configs,interface,antigen_feat)
...
def _normalize_conditioning_tensor(value):
    if isinstance(value, list):
        if not value:
            raise ValueError("Conditioning tensor list is empty")
        tensors = []
        for item in value:
...
def iterative_sampling_tokens(
    ...
    configs: list[GenerationConfig],
    tokenizers: TokenizerCollectionProtocol,
    interface: torch.Tensor | list[torch.Tensor],
    antigen_feat: torch.Tensor | list[torch.Tensor]
) -> Sequence[ESMProteinTensor | ESMProteinError]:
    interface = _normalize_conditioning_tensor(interface)
    antigen_feat = _normalize_conditioning_tensor(antigen_feat)
    ...
    for t in tqdm(range(max_num_steps), disable=disable_tqdm):
        forward_out = _batch_forward(client, batched_tokens,interface,antigen_feat)
...
def _batch_forward(client: ESM3InferenceClient, protein: _BatchedESMProteinTensor,interface: torch.Tensor,antigen_feat: torch.Tensor):
    # Forward pass
